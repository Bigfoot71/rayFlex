...
/*
    ...
    TODO: Also make sure that endianness is respected
*/
...
namespace rf { namespace net {
     ...
     */
    template<typename T_PacketID>
    class Connection : public std::enable_shared_from_this<Connection<T_PacketID>>, private HandshakeHandler
    {
      public:
      ...
      protected:
        ...
        std::unique_ptr<CryptoHandler> cryptoHandler;       ///< Encrypted communication handler
        TSQueue<Packet<T_PacketID>> packetsOut;             ///< This queue holds all messages to be sent to the remote side of this connection
        TSQueue<OwnedPacket<T_PacketID>>& packetsIn;        ///< This references the incoming queue of the parent object
        ...
        uint32_t id = 0;                                    ///< ID of this connection that the server will give for example

    public:
        // Constructor: Specify Owner, connect to context, transfer the socket. Provide reference to incoming Packet queue
        Connection(Owner parent, asio::io_context& asioContext, asio::ip::tcp::socket socket, TSQueue<OwnedPacket<T_PacketID>>& qIn)
         ...
         */
        void ConnectToClient(ServerInterface<T_PacketID>* server, uint32_t uid);
      ...
      private:
        ...
        void ReadValidationFromClient(ServerInterface<T_PacketID>* server, const KeyPair& serverKeyPair);       ///< ASYNC - For the server - TODO: Explain
    ...
    template<typename T_PacketID>
    void Connection<T_PacketID>::ConnectToClient(ServerInterface<T_PacketID>* server, uint32_t uid)
    {
    ...
    template<typename T_PacketID>
    void Connection<T_PacketID>::ReadValidationFromClient(ServerInterface<T_PacketID>* server, const KeyPair& serverKeyPair)
    {
        if (ownerType == Owner::SERVER)
        {
            ...
            asio::async_read(socket, asio::buffer(&handshakeIn, sizeof(handshakeIn)),
                [this, server, serverKeyPair](std::error_code ec, std::size_t length)
                {
                    if (!ec)
                    {
                        ...
                        uint8_t clientPublicKey[CryptoKeySize];
                        ...
                        if (IsClientHandshakeValid(clientPublicKey))
                        {
                            ...
                            cryptoHandler = std::make_unique<CryptoHandler>(
        ...
        if (ownerType == Owner::CLIENT)
        {
            asio::async_read(socket, asio::buffer(&handshakeIn, sizeof(handshakeIn)),
                [this](std::error_code ec, std::size_t length)
                {
                    if (!ec)
                    {
                        KeyPair clientKeyPair;                          // Generate a temporary key pair for the final pair generation
                        uint8_t serverPublicKey[CryptoKeySize];      // Buffer to store the server's public key
                        ...
                        cryptoHandler = std::make_unique<CryptoHandler>(clientKeyPair, serverPublicKey, false);
                    }
                    else
                    {
                        // A significant failure occurred.
